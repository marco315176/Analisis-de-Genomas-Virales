...
echo -e ===== Evaluación de profundidad de los ensambles obtenidos =====
...
for ensamble in *fa; do
    ensamble_ID="${ensamble%%-metaSPAdes-assembly.fa}"
...
paste /home/secuenciacion_cenasa/Analisis_corridas/Resultados_fastQC/Virus/estadisticos/lecturas_stats.tsv /home/secuenciacion_cenasa/Analisis_corridas/Resultados_fastqc_ptrim/Virus/estadisticos/lecturas_stats_pt.tsv ./estadisticos/Estadisticos_totales.tsv |  awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$5"\t"$6"\t"$7"\t"$8"\t"$9"\t"$11"\t"$12"\t"$13"\t"$14"\t"$15"\t"$16"\t"$17"\t"$18"\t"$20"\t"$21"\t"$22"\t"$23}' > ./estadisticos/Estadistico_global.tsv
