...
echo -e ===== Correr BLASTn para identificar contigs de genes de Influenza y rabia en ensambles obtenidos con SPAdes ===== "\n"
...
#/home/secuenciacion_cenasa/Analisis_corridas/SPAdes_viral
...
blastn -query ${ensamble} -db $BLASTN_DB_PATH/virus_db -outfmt "6 qseqid salltitles" -max_target_seqs 1 -perc_identity 90 -evalue 1e-10 -out /home/secuenciacion_cenasa/Analisis_corridas/SPAdes_viral/BLASTn_results/${ID}_results.tsv
...
paste ./BLASTn_results/${ID}_nodos.txt ./BLASTn_results/${ID}_gen3.txt > ./BLASTn_results/${ID}_BLASTn_results_tmp.tsv
...
# --------------------------------------------------------
# Conjuntar los archivos resultantes del BLAST en uno solo
# --------------------------------------------------------

cd /home/secuenciacion_cenasa/Analisis_corridas/SPAdes_viral/BLASTn_results

for file in *_BLASTn_results*; do
    ename="$(basename ${file} | cut -d '_' -f '1')"
    echo -e "\n${ename} \n$(cat ${file})"

done >> ./Blastn_all_results.tsv
